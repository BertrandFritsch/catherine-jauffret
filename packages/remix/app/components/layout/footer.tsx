...
export default function Footer({ className, socialMedias, overlay }: Props) {
	return (
		<footer className={classnames(className, 'flex justify-center')}>
			<motion.div
				className={classnames(
					'flex flex-col items-center gap-2 border-t border-solid border-[var(--color-default-foreground)] px-8 pt-4 pb-4 text-xs',
					{
						'fixed bottom-0 z-[1] bg-[var(--bg-overlay-bottom)]': overlay,
					},
				)}
