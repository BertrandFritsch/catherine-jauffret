	...
	return (
		...
		>
			<AnimatePresence>
				{isOpen && (
					<motion.div
						className="fixed top-0 z-10 h-screen w-screen bg-[var(--color-root-background)]"
			...
			<motion.div
				className={classnames(
					'fixed top-0 right-0 left-0 z-10 flex bg-[var(--color-root-background)]',
			...
			>
				<AnimatePresence>
					{isOpen && (
						<motion.div
							className="bg-root-background absolute top-0 left-0 flex h-[var(--spacing-mobile-menu)] w-full justify-center rounded-bl-[var(--rounded-menu-corner)] pt-[var(--spacing-header-mobile)]"
				...
				>
					<span className="hamburger-box">
						<span className="hamburger-inner bg-default-foreground" />
					</span>
				</button>
			</motion.div>
